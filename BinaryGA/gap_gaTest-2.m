function process_large_gap_ga()
    ...
    for file_idx = 1:num_files
        filename = sprintf('/MATLAB Drive/gap dataset files/gap%d.txt', file_idx);
...
function run_gap12_binary_ga_average_profits()
    % Input and output
    gap_file = '/MATLAB Drive/gap dataset files/gap12.txt';
...
function plot_gap12_convergence()
    gap_file = '/MATLAB Drive/gap dataset files/gap12.txt';
    fid = fopen(gap_file, 'r');
    if fid == -1
        error('Cannot open file: %s', gap_file);
    ...
    num_instances = fscanf(fid, '%d', 1);
    max_generations = 300;
    all_curves = zeros(num_instances, max_generations);
    ...
    for instance = 1:num_instances
        ...
        [~, convergence_curve] = optimize_gap_ga(num_servers, num_users, cost_matrix, resource_matrix, capacity_vector);
        all_curves(instance, :) = convergence_curve;
    end
    fclose(fid);
    ...
    figure;
    plot(1:max_generations, avg_curve, 'LineWidth', 2);
    xlabel('Generation');
    ylabel('Best Fitness');
    title('Convergence Curve for gap12 (Average Across Instances)');
    grid on;
end
